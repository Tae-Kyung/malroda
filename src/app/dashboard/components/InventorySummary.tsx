...
export default async function InventorySummary({ userId }: { userId: string }) {
    ...
    if (!farmId) return <div>농장 정보가 없습니다.</div>
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden h-full flex flex-col">
            <div className="p-5 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                ...
                <span className="text-xs font-medium bg-emerald-100 text-emerald-800 px-2 py-1 rounded-full">{totalItems}가지 품목</span>
            </div>

            <div className="p-5 flex-1">
                <div className="flex justify-between items-end mb-6">
                    <div>
                        <p className="text-sm text-gray-500 mb-1">총 보유 재고</p>
                ...
                {topItems.length > 0 ? (
                    <div>
                        <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-3">상위 보유 품목</h4>
                        <div className="space-y-3">
                            {topItems.map((item, idx) => (
                                <div key={idx} className="flex justify-between items-center text-sm">
                                    <span className="text-gray-700 font-medium truncate pr-4">{item.name}</span>
                                    <span className="text-gray-900 font-semibold">{item.stock.toLocaleString()}</span>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            <div className="bg-gray-50 p-3 mt-auto border-t border-gray-100 text-center">
                <a href="/dashboard/settings" className="text-sm text-emerald-600 font-medium hover:text-emerald-700">전체 재고 설정 관리 &rarr;</a>
            </div>
        </div>
    )
}
